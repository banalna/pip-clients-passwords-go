...
func (c *PasswordsMemoryClientV1) SetTempPassword(correlationId string, userId string) (password string, err error) {
	...
	password = "pass" + cconv.StringConverter.ToString(crnd.RandomInteger.NextInteger(0, 1000))
...
func (c *PasswordsMemoryClientV1) Authenticate(correlationId string, userId string, password string) (authenticated bool, err error) {
	...
	userPassword, err := c.readUserPassword(correlationId, userId)
...
func (c *PasswordsMemoryClientV1) ChangePassword(correlationId string, userId string,
	...
	verify, err := c.verifyPassword(correlationId, newPassword)
	if !verify || err != nil {
		return nil
	}
...
func (c *PasswordsMemoryClientV1) ValidateCode(correlationId string, userId string,
	...
	data, err := c.readUserPassword(correlationId, userId)
...
func (c *PasswordsMemoryClientV1) ResetPassword(correlationId string, userId string, code string, password string) error {
	...
	verify, err := c.verifyPassword(correlationId, password)
	if !verify || err != nil {
		return nil
	}
...
func (c *PasswordsMemoryClientV1) readUserPassword(correlationId string, userId string) (result *UserPasswordV1, err error) {
...
func (c *PasswordsMemoryClientV1) verifyPassword(correlationId string, password string) (res bool, err error) {

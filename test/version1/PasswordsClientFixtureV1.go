...
func (c *PasswordsClientFixtureV1) clear() {
	c.Client.DeletePassword("", USER_PWD.Id)
}
...
func (c *PasswordsClientFixtureV1) TestChangePassword(t *testing.T) {
	...
	// Create a new user
	err := c.Client.SetPassword("", USER_PWD.Id, USER_PWD.Password)
	assert.Nil(t, err)

	// Recover password
	err = c.Client.RecoverPassword("", USER_PWD.Id)
	assert.Nil(t, err)
}
...
func (c *PasswordsClientFixtureV1) TestRecoverPassword(t *testing.T) {
	...
	// Change password
	err = c.Client.ChangePassword("", USER_PWD.Id, USER_PWD.Password, "xxx123")
	assert.Nil(t, err)

	// Sign in with new password
	auth, err1 := c.Client.Authenticate("", USER_PWD.Id, "xxx123")
	assert.Nil(t, err1)
	assert.True(t, auth)

	// Delete password
	err = c.Client.DeletePassword("", USER_PWD.Id)
	assert.Nil(t, err)
}
...
func (c *PasswordsClientFixtureV1) TestSigninWithWrongPassword(t *testing.T) {
	...
	auth, err := c.Client.Authenticate("", USER_PWD.Id, "xxx")
	assert.NotNil(t, err)
	assert.False(t, auth)
}
